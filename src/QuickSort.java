...
public class QuickSort<T extends Comparable<T>> {
    ...
    public static <T extends Comparable<T>> T[] sort(PIVOT_TYPE pivotType, T[] unsorted) {
        ...
        if (pivotType == PIVOT_TYPE.MIDDLE) {
            ...
            pivot = getRandom(unsorted.length); //Fix: Removed +1
        }
    ...
    private static <T extends Comparable<T>> void sort(int index, int start, int finish, T[] unsorted) {
        if (start >= finish) return; //Fix: Added base case to prevent invalid recursion
        ...
        int f = finish;
        while (s <= f) {
            while (s <= finish && unsorted[s].compareTo(pivot) < 0) s++; //Fix: Added bounds check
            while (f >= start && unsorted[f].compareTo(pivot) > 0) f--; //Fix: Added bounds check
            if (s <= f) {
                swap(s, f, unsorted);
                s++;
                f--;
